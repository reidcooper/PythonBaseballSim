...
# This code processes the FanGraphs csv files into teamText files which will be used by BASES
...
def printBatting(team):
    ...
    with open('FanGraphsBatting/2014_batting_master_withoutPercentage.csv', 'rU') as csvFileBatting:
        ...
        for row in readerBatting:
            if (row["Team"] == team):
                batting_players.append([row["Name"].replace(" ", ""), row["Position"], row["H"], float(row["IPR"]), float(row["O-Swing%"]), float(row["Z-Swing%"]), float(row["O-Contact%"]), float(row["Z-Contact%"]), float(row["1B%"]), float(row["2B%"]), float(row["3B%"]), float(row["HR%"]), float(row["FP"]), int(row["G"])])


    batting_output = sorted(batting_players, key=itemgetter(13), reverse=True)
...
def printPitching(team):
    ...
    with open('FanGraphsPitching/2014_pitching_master_withoutPercentage.csv', 'rU') as csvFilePitching:
        ...
        for row in readerPitching:
            if (row["Team"] == team):
                pitching_players.append([row["Name"].replace(" ", ""), int(row["G"]), "P", (float(row["Zone%"])), (float(row["FB%"])), (float(row["FBv"])), (float(row["SL%"])), (float(row["SLv"])), (float(row["CT%"])), (float(row["CTv"])), (float(row["CB%"])), (float(row["CBv"])), (float(row["CH%"])), (float(row["CHv"])), (float(row["SF%"])), (float(row["SFv"])), (float(row["KN%"])), (float(row["KNv"]))])
