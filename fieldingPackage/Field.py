 ...
 def newPlayerOnBases( n, p, numOfOuts,walkOrHomeRun):
  ...
  if walkOrHomeRun == "walks":
	...
	try:
	 ...
	 if n > 0:	
	       if not(currentField.three.getPlayerOnBase() == null):
	              playerOnThrid()
		
		
	       if not(currentField.two.getPlayerOnBase() == null):
		...
		if not (currentField.one.getPlayerOnBase() == null):
                       if n == 1:
